/**
 * OmpDriver — `ProviderDriver` for the omp (Oh My Pi) runtime.
 *
 * omp ships a first-party ACP server (`omp acp`). The model catalogue is
 * discovered during the managed provider status check by reading the ACP
 * `configOptions` entry whose category is `model`.
 *
 ...
 *
 * omp is distributed on its own release cadence and exposes no in-CLI update
 * command, so maintenance is manual-only.
 *
