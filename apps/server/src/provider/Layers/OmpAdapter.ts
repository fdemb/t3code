/**
 * OmpAdapterLive — omp (Oh My Pi) via its ACP server (`omp acp`).
 *
 * Adapted from the Cursor adapter, which shares the same ACP session
 * runtime. omp implements only the stable ACP surface — it ships no
 * proprietary extension methods — so the `cursor/*` ext-request handlers
 * have no counterpart here and the raw event source is always
 * `acp.jsonrpc`.
 *
...
export interface OmpAdapterLiveOptions {
   ...
   * Defaults to the built-in instance id (`omp`).
   */
...
/**
 * omp model slugs are `provider/model` pairs and carry no bracketed suffix,
 * so resolving one is just a trim. `default` means "whatever the active omp
 * profile is configured to use".
 */
function resolveOmpBaseModelId(model: string | null | undefined): string {
...
function applyRequestedSessionConfiguration<E>(input: {
  ...
  return Effect.gen(function* () {
    if (input.modelSelection) {
      yield* input.runtime.setModel(resolveOmpBaseModelId(input.modelSelection.model)).pipe(
  ...
  return Effect.gen(function* () {
        ...
        Effect.mapError(
          (cause) =>
            new EffectAcpErrors.AcpTransportError({
              detail: "Failed to process omp ACP event.",
              cause,
            }),
        ),
      );
    ...
    const sendTurn: OmpAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          ...
          const resolvedModel = resolveOmpBaseModelId(model);
          yield* applyRequestedSessionConfiguration({
            runtime: ctx.acp,
    ...
    yield* Effect.addFinalizer(() =>
      Effect.forEach(sessions.values(), stopSessionInternal, { discard: true }).pipe(
        Effect.catch((cause) =>
          Effect.logError("Failed to emit omp session shutdown event.", { cause }),
        ),
